...
public class MenuItemController {
    ...
    @GetMapping("/all")
    public ResponseEntity<List<MenuItem>> getAllMedicine() {
        List<MenuItem> response = menuItemService.findAll();
        return ResponseEntity.ok(response);
    }

    @GetMapping("/id/{id}")
    public ResponseEntity<MenuItem> getMedicineById(@PathVariable Integer id) {
        MenuItem response = menuItemService.findById(id);
        return ResponseEntity.ok(response);
    }

    @PostMapping("/create")
    public ResponseEntity<MenuItem> addMedicine(@RequestBody MenuItemRequest request) {
        MenuItem response = menuItemService.create(request);
        return ResponseEntity.ok(response);
    }

    @PutMapping("/update/{id}")
    public ResponseEntity<MenuItem> putMedicine(@PathVariable Integer id, @RequestBody MenuItemRequest request) {
